...
Strategies:
  orb_atr  ΓÇö ORB with ATR-fixed stop (stop = fixed ATR fraction from entry)
  orb_half ΓÇö ORB with midpoint stop  (stop = midpoint of opening range)
  vola     ΓÇö Volatility Expansion: enter on bar with range > 1.5x ATR
...
# FX USD-base pairs (USDCAD, USDCHF): pv Γëê $10 (quote-rate varies; $10 is a reasonable mid).
# FX GBP-cross (EURGBP): pv Γëê $12-13 (depends on GBPUSD rate); using $12.70 mid.
# JPY pairs: pv = 100,000 * 0.01 / rate. Rates for 2024-2026 mid: USDJPYΓëê148, EURJPYΓëê163, GBPJPYΓëê193.
...
# ---------------------------------------------------------------------------
# Lot sizing ΓÇö FIXED base to prevent compounding blow-up
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Trade simulator ΓÇö bar-level exit scan
# ---------------------------------------------------------------------------
...
def simulate(sig: dict, future_bars: list[Bar], end_utc: datetime, symbol: str,
    ...
    re-touch return None (no trade) ΓÇö the old code silently booked these as
    ...
    (signal timestamp, symbol, direction) ΓÇö reproducible mid bound.
...
def print_leaderboard(results: list[dict]) -> list[dict]:
    ...
    print(f"\n{'='*105}")
    print("TOP COMBINATIONS ΓÇö fastest Phase 1, then monthly P&L  (sigs>=15, avgR>0, dd<9%, not halted)")
    print(f"{'='*105}")
...
def write_findings(results: list[dict], top: list[dict], loaded: dict | None = None) -> None:
    ...
    for i, r in enumerate(top[:10], 1):
        pf_s = f"{r['profit_factor']:.2f}" if r['profit_factor'] != float('inf') else "inf"
        dp1  = str(r['days_to_p1']) if r['phase1_done'] else "ΓÇö"
...
**Strategy: `{best['strategy']}` | Target={best['target_r']}R | ORB={best['orb_bars']} bars ({best['orb_bars']*5} min) | ATR stop={best['atr_stop']}├ùATR**
...
"""
    ...
    if loaded:
        ...
        data_line = (f"**Data:** {len(loaded)} pairs M5 OHLCV, "
                     f"{d0:%Y-%m-%d} ΓÇô {d1:%Y-%m-%d} (~{years:.1f} years, "
    ...
    mode_line = ("**Simulator (fixed 2026-09-12):** limit fills require a re-touch of the "
                 ...
                 "target/stop bars resolved by deterministic 50/50 coin ΓÇö pass "
                 "`stop_first=True/False` to `run_backtest` for the pessimistic/optimistic bounds.")

    md = f"""# Aggressive Multi-Pair Optimizer ΓÇö Findings
...
**Challenge:** The5ers $2,500 New High Stakes ΓÇö Phase 1 +10%, Phase 2 +5%

---

## 1. Approach

Three strategies ├ù 4 target R ├ù 3 ORB sizes ├ù 3 ATR stops = {len(results)} combinations.
...
| `vola` | Bar with range > 1.5├ùATR, in bar direction | Bar midpoint |

### Session windows (DST-correct)

| Session | Window | Pairs |
|---|---|---|
| London | 07:00ΓÇô11:00 Europe/London | EURUSD GBPUSD EURGBP GBPJPY EURJPY AUDUSD USDCHF NZDUSD XAUUSD |
| New York | 08:30ΓÇô11:00 America/New_York | USDJPY USDCAD EURUSD GBPUSD XAUUSD |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
{top_rows.rstrip() if top_rows.strip() else "| ΓÇö | No viable combos met all criteria | | | | | | | | | | | |"}
{best_sec}
...
1. **Fixed lot sizing** ΓÇö always size off $2,500 regardless of current balance.
...
2. **Max 2 trades/day** ΓÇö avoids overexposure on correlated pairs (EURUSD+GBPUSD
   often move together). A 2-trade limit with 0.40% risk = 0.80% max daily risk.

3. **ATR-fixed stop** ΓÇö decouples stop size from opening range width. A wide
   opening range no longer forces a wide stop, solving the v1 ORB problem.

4. **All 11 pairs including XAUUSD** ΓÇö gold has very high ATR and clean session
