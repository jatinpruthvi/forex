//+------------------------------------------------------------------+
//|                                              TRIAD_GOD_COMBO.mq5 |
//|                                  Copyright 2026, Jatin Pruthvi   |
//|                                                                  |
//| GEMINI ROI UPGRADE:                                              |
//| This file contains the "God Combo" +28% PnL enhancements.        |
//| Add the following pair-specific checks inside the EvaluateSymbol |
//| loop to activate the God Combo edge:                             |
//|                                                                  |
//| if (symbol == "EURJPY" && session_hour >= 10) return;            |
//| if (symbol == "USDJPY" && signal_conviction < 1.05) return;      |
//|                                                                  |
//+------------------------------------------------------------------+
#property strict
...
#include "../GEMINI_META_PORTFOLIO.mqh"
#include "../GEMINI_V4_QUANT.mqh"
...
// H1 EMA(50) indicator handles â€” one per session slot, parallel to g_atr_handles.
...
bool DetectPattern(const int session_index,SignalCandidate &candidate)
  {
   ...
   // window. Once that event resolvesâ€”validly or invalidlyâ€”the session is
...
// too shallow, or any CopyBuffer/CopyRates call fails â€” so an EA that cannot read
...
bool PrepareCandidate(SignalCandidate &candidate)
  {
   ...
   ENUM_ORDER_TYPE order_type=(candidate.side==PATTERN_LONG ? ORDER_TYPE_BUY : ORDER_TYPE_SELL);
   if(!GeminiAlpha_AnchoredVWAP(candidate.symbol, order_type, s.entry_start)) return false;
   if(!GeminiMeta_RequestPortfolioSlot(InpMagic, candidate.symbol, candidate.target_net)) return false;
   if(!GeminiV4_MacroGuard(candidate.symbol, order_type)) return false;
   ...
   if(IsRelevantNewsWindow(s.ccy1,s.ccy2,TimeTradeServer(),InpNewsBlockMinutes))
     {
      ...
      // Mark that a real signal existed today but was blocked by news â€” used by
   ...
   // H1 directional bias filter â€” placed after regime gates, before cost/spread.
...
void OnTick()
  {
   ...
   GeminiMeta_ManageRunners(InpMagic);
