...
void OnTick()
  {
   ...
   if(!GeminiMeta_RequestPortfolioSlot(InpMagic, Symbol(), 1.0)) return;
   if(!GeminiV4_MacroGuard(Symbol(), type)) return;
   ...
   req.symbol = Symbol();
   req.volume = 0.01 * GeminiMeta_GetEdgeWeightedRisk(Symbol(), PERIOD_D1, InpMagic) * GeminiV4_EquityCurveSizing(InpMagic) * GeminiV4_FVG_Score(Symbol(), type);
   req.type = type;
