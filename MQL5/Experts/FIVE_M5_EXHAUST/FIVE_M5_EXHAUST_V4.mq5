...
void EvaluateSymbol(const int idx,const datetime now)
  {
   ...
   if(!GeminiAlpha_VolumeCapitulation(sym)) return;   // GEMINI V2 ALPHA: Require Volume Climax
   ...
   if(!GeminiMeta_RequestPortfolioSlot(InpMagic, sym, 1.0)) return;
   if(!GeminiV4_MacroGuard(sym, ORDER_TYPE_BUY)) return;
